...
function reset_board() {
	$.ajax({url: "index1.php/board/", headers: {"X-Token": me.token}, method: 'POST',  success: fill_board_by_data });
	$('#move_div').hide();
...
function login_to_game() {
	...
	$.ajax({url: "index1.php/players/"+p_color, 
			method: 'PUT',
			dataType: "json",
			headers: {"X-Token": me.token},
			contentType: 'application/json',
...
function game_status_update() {
	$.ajax({url: "index1.php/game_status/", success: update_status,headers: {"X-Token": me.token} });
}
...
function update_info(){
	$('#game_info').html("I am Player: "+me.piece_color+", my name is "+me.username +'<br>Token='+me.token+'<br>Game state: '+game_status.status+', '+ game_status.p_turn+' must play now.<br>'
	+game_status.result+"   player is the big winner");
	}
